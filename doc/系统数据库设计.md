...
|status |varchar(45) |YES |NO |可用状态 |available/unavailable |
|||||||
...
|role  |enum('admin','staff','customer') |YES  |NO|用户角色 |角色锁死，不可新增 |
|userid |int |YES |NO |关联ID |该账户对应的ID |
|status |varchar(45) |YES |INDEX4 |客户类型 |根据需求进行定义 |
|||||||
...
| pid | varchar(45) | YES | INDEX1 |产品编号 |  |
|code|varchar(45)  |YES  |UNIQUE  |设备编号 | 唯一标识 |
|status |varchar(45) |YES |NO |可用状态 |available/unavailable |
| | |  |  ||  |
...
|insurance |varchar(45) |YES |NO |损坏保险 | |
...
|tip_time  |varchar(45)  |YES  |NO|巡检时间点 | |
...
* 说明：
  ...
  * 投诉状态在数据库中不做限制，但建议应用层应按照“未受理、受理中、已完成、已取消”四种状态储存。
...
| timestamp | DATETIME | YES | INDEX2 |投诉时间|  |
|status |varchar(45) |YES |NO |投诉状态 | |
| | | | || |

## 系统模块

请保证日志记录写入的一致性与安全性，相关表不添加标识删除的字段。
